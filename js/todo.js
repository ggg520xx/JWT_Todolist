...
function start() {
  axios
    .get(`${url}/todos`, { headers: { authorization } })
...
function addTodo() {
  ...
  let todo = {
    content: inputText.value, // 輸入事項
    id: new Date().getTime(), // 抓獨特項目 刪除或切換狀態用的
    completed_at: null, // 判別已完成 待完成項目的
  };
  ...
  if (todo.content != "") {
    ...
    todoData.push(todo); // 點擊監聽事件就執行 把todo這包物件從陣列前面塞進去

    axios
      .post(`${url}/todos`, todo, { headers: { authorization } })
      .then(function (response) {
...
function deleteAndChecked(e) {
  ...
  if (e.target.classList.value == "delete") {
    ...
    axios
      .delete(`${url}/todos/${id}`, { headers: { authorization } })
      .then(function (response) {
        // id一樣的被過濾掉 就等於刪除的意思
    ...
    todoData.forEach(function (item, index) {
      if (item.id == id) {
        // 若陣列那筆 等同我點到的那筆
        if (todoData[index].completed_at == "checked") {
          // 那筆的checked狀態 如果點到的那筆在陣列內的值已經是有checked的話
          // 我們就拿掉 變為沒有任何打勾狀態
          todoData[index].completed_at = null;

          // patch方法用来更新局部资源;
          axios
            .patch(
              `${url}/todos/${id}/toggle`,
              todoData[index], { headers: { authorization } }
            )
            .then(function (response) {
          ...
          todoData[index].completed_at = "checked";
...
function updateList() {
  ...
  if (toggleStatus == "all") {
    ...
    todoData.forEach(function (item, index) {
      ...
      if (typeof item.completed_at == 'string') {
...
clearFin.addEventListener("click", function (e) {
  ...
  const finished = todoData.filter((item) => item.completed_at !== null);
  finished.forEach(function (item, index) {
    todoData = todoData.filter(function (item) {
      return typeof item.completed_at != 'string';
    });

    axios
      .delete(
        `${url}/todos/${item.id}`, { headers: { authorization } }
